...
void insereOrdenado(tipoRegistro *registro,int novaInsc,int novaNota)
{
    tipoCadastro *novo
    tipoCadastro *ant=NULL;  /* ponteiro para elemento anterior */
    tipoCadastro *p=registro->inicio;/* ponteiro para percorrer a lista */

    while(p != NULL ||  p->nota >= novaNota && (p->insc<=novoInsc)))          /* procura posição de inserção */
    {
        ant=p;
        p=p->prox;
    }
...
int main()
{
    ...
    for(i=0; i<n; i++)
    {
        ...
        case 1:
            ...
            insereOrdenado(&registros,cadastros.insc,cadastros.nota);
            break;
        case 2:
